...
const hasRealIndentId = (indentId) =>
  indentId && indentId.trim() !== "" && indentId.trim() !== "—";
...
function POApprovalModal({ open, onClose, po, onConfirm }) {
  ...
  const [approvedQty, setApprovedQty] = useState(() =>
    Object.fromEntries(
      items.map((it, i) => [
        i,
        it.approvedQty ?? it.indentApprovedQty ?? it.quantity,
      ]),
    ),
  );
  const [reason, setReason] = useState(() =>
    Object.fromEntries(items.map((it, i) => [i, it.approvalReason || ""])),
  );
  ...
  const handleConfirm = () => {
    ...
    const missingReason = selected.some((idx) => {
      const indentApproved =
        items[idx].indentApprovedQty ?? items[idx].quantity;
      return (
        Number(approvedQty[idx] ?? indentApproved) < indentApproved &&
        !reason[idx]?.trim()
      );
    });
    ...
    const updatedItems = items.map((item, idx) => {
      if (!selected.includes(idx))
        return { ...item, approvalStatus: "Rejected", approvedQty: 0 };
      const indentApproved = item.indentApprovedQty ?? item.quantity;
      const appQty = Number(approvedQty[idx] ?? indentApproved);
      return {
        ...item,
        approvalStatus: appQty < indentApproved ? "Partial" : "Approved",
        approvedQty: appQty,
        approvalReason: reason[idx] || "",
      };
    });

    const allApproved = updatedItems.every(
      (it) => it.approvalStatus === "Approved",
    );
    const allRejected = updatedItems.every(
      (it) => it.approvalStatus === "Rejected",
    );
    const hasPartial = updatedItems.some(
      (it) => it.approvalStatus === "Partial",
    );
    ...
    const newStatus = allRejected
      ...
      : allApproved
        ? "Approved"
        : hasPartial || hasRejected
          ? "Partial Approved"
          : "Approved";
    const newTotal = updatedItems
      .filter((it) => it.approvalStatus !== "Rejected")
      .reduce((s, it) => s + (it.approvedQty ?? it.quantity) * it.unitCost, 0);
    ...
    >
      ...
      >
        ...
        >
          <Box />
          <Box />
          {["Description", "Indent Approved", "Approve", "Reason"].map(
            (h, i) => (
              <Typography
                key={h}
                sx={{
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: "5px" }}>
          {items.map((item, idx) => {
            ...
            const appQty = approvedQty[idx] ?? indentApproved;
            const isPartial = isChecked && Number(appQty) < indentApproved;
            return (
              <Box key={idx}>
                <Box
                  onClick={() => toggle(idx)}
                  sx={{
                    ...
                    bgcolor: isChecked ? "#f0fdf4" : "#f9fafb",
                    cursor: "pointer",
                    transition: "all 0.12s",
                    "&:hover": { borderColor: "#bbf7d0", bgcolor: "#f0fdf4" },
                  }}
                >
                  <Checkbox
                    size="small"
                  ...
                  <Box sx={{ textAlign: "center" }}>
                    <Typography sx={{ fontSize: 12.5, fontWeight: 700, color: "#7c3aed" }}>
                      {item.indentApprovedQty ?? item.quantity}
                    </Typography>
                  ...
                  >
                    <TextField
                      ...
                      onChange={(e) => {
                        const val = Math.max(
                          0,
                          Math.min(Number(e.target.value) || 0, indentApproved),
                        );
                ...
                {isPartial && (
                  ...
                  >
                    ...
                    <Typography sx={{ fontSize: 11, color: "#92400e", fontWeight: 600 }}>
                      Partial: approving {appQty} of {item.quantity}
                    </Typography>
                  </Box>
                )}
              </Box>
...
const ViewPOModal = ({ open, onClose, po, onSave }) => {
  ...
  // ── Updated priority colors inside ViewPOModal ──
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: { xs: 0.5, sm: 0.75, md: 1 } }}>
            {displayItems.map((item, idx) => {
              ...
              return (
                ...
                >
                  <Box sx={{ display: "flex", alignItems: "center", gap: { xs: 0.5, sm: 0.75, md: 1 }, minWidth: 0 }}>
...
function POStatusChip({ po, statusStyle }) {
  ...
  const getTooltipText = () => {
    if (po.status === "Partial Approved") {
      const approved = items.filter((it) => it.approvalStatus === "Approved").length;
      const partial = items.filter((it) => it.approvalStatus === "Partial").length;
      const rejected = items.filter((it) => it.approvalStatus === "Rejected").length;
      return `Status: ${po.status} (${approved} approved, ${partial} partial, ${rejected} rejected)`;
    }
...
const PurchaseOrders = () => {
  ...
  const [poApprovalModal, setPoApprovalModal] = useState({ open: false, po: null });
  ...
  const { suppliers, supplierNames } = useVendorManagement();
  ...
  const filteredPOs = purchaseOrders.filter((po) => {
    ...
    const indent = hasRealIndentId(po.indentId) ? po.indentId.toLowerCase() : "";
  ...
  const handleApprove = (po) => setPoApprovalModal({ open: true, po });

  const handleApprovalConfirm = (updatedPO) => {
    setPurchaseOrders((prev) => prev.map((p) => (p.id === updatedPO.id ? updatedPO : p)));
    const isPartial = updatedPO.status === "Partial Approved";
    setToast({
      msg: (
        <span>
          PO <strong>{updatedPO.id}</strong>{" "}
          {isPartial ? "partially approved" : updatedPO.status === "Rejected" ? "rejected" : "approved"}.
        </span>
      ),
      type: isPartial ? "info" : updatedPO.status === "Rejected" ? "error" : "success",
    });
  ...
  const handleReject = (id) => {
    ...
    setToast({ msg: (<span>PO <strong>{id}</strong> rejected.</span>), type: "error" });
    setTimeout(() => setToast(null), 4000);
  };
  ...
  const getOpenPOBalance = (po) => {
    ...
    return (po.lineItems || []).reduce((total, item) => {
      ...
      const orderedQty = parseFloat(item.quantity ?? item.qty ?? 0) || 0;
      const receivedQty = receivedByItem[key] || 0;
      return total + Math.max(0, orderedQty - receivedQty);
    }, 0);
  };
  ...
  const handleMailPOConfirm = (mailData) => {
    try {
      ...
      console.error("Failed to mark PO as mailed:", e);
    }
    setToast({
  ...
  const handleSavePO = (poData) => {
    ...
    const newPO = {
      id: poData.poNumber,
      indentId: hasRealIndentId(poData.indentId) ? poData.indentId.trim() : "—",
    ...
    setToast({
      msg: (
        <span>
          PO <strong>{poData.poNumber}</strong> {isDraft ? "saved as draft" : "submitted successfully"}!{" "}
          <a
    ...
    <Box>
      ...
      >
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
          ...
          {/* ── Priority Filter — updated to Low / Medium / High / Critical ── */}
          <Box sx={{ minWidth: 148 }}>
            <Select
                  ...
                  >
                    ...
                    <TableCell sx={{ ...tdSx, minWidth: 100 }}>
                      <Box sx={{ display: "flex", gap: { xs: 0.25, sm: 0.5, md: 0.75 }, flexWrap: "wrap" }}>
                        ...
                        {po.status === "Pending" && (
                          <>
                            <Tooltip title={!can.approvePOItems ? "You don't have permission to approve purchase orders" : "Approve"}>
                              <span>
                                <IconButton
                                  size="small"
                            ...
                            <Tooltip title={!can.rejectPOItems ? "You don't have permission to reject purchase orders" : "Reject"}>
                              <span>
                                <IconButton
                                  size="small"
                        ...
                        {po.status === "Partial Approved" && (
                          <Tooltip title={!can.approvePOItems ? "You don't have permission to approve purchase orders" : "Upgrade to Full Approval"}>
                            <span>
                              <IconButton
                                size="small"
                                onClick={() => handleApprove(po)}
                                disabled={!can.approvePOItems}
                                sx={{
                                  color: "#2563eb", border: "1px solid #bfdbfe", borderRadius: "6px",
                                  width: { xs: 26, sm: 28, md: 28 }, height: { xs: 26, sm: 28, md: 28 }, bgcolor: "#eff6ff",
                                  "&:hover": { background: "#dbeafe", color: "#1d4ed8", borderColor: "#93c5fd" },
                                  "&:disabled": { opacity: 0.5, color: "#2563eb", bgcolor: "#eff6ff" },
                                }}
                              >
                                <CheckIcon sx={{ fontSize: { xs: 12, sm: 14, md: 14 } }} />
                              </IconButton>
      ...
      {/* ── Modals ── */}
      <POApprovalModal open={poApprovalModal.open} onClose={() => setPoApprovalModal({ open: false, po: null })} po={poApprovalModal.po} onConfirm={handleApprovalConfirm} />
